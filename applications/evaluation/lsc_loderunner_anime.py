...
# This argument can be a little confusing, so here is an example showing how
# this works.
# Assume the initial image is at time zero.
# Assume you are trying to predict the n'th timestep.
# This would mean that you are most likely trying to predict time k * n, for
# some constant k.
# In single mode, this prediction would be made by taking the TRUE input for
# the n-1 timestep.
#     The model would then use that, along with timestep k to predict
#     timestep n.
...
#             Can be used on models trained to only predict one timestep
#             ahead.
#         Disadvantages:
...
#                 Predicting timestep n requires n calls to the model.
#                 (O(n) model calls)
#             Predictions lose accuracy across the rollout.
#                 The prediction quality decays significantly as the rollout
#                 progresses forward.
#             Uses a constant dt, so some parameters may be ignored.
#     Timestep Mode
#         Advantages:
...
#             Allows for predictions of a specific timestep to only require
#             one model call. (O(1) model calls)
#         Consideration:
#             Requires a model trained with a variety of timesteps.
#                 Training the model to predict one timestep ahead
#                 (exclusively) will NOT work effectively in this mode.
parser.add_argument(
    "--mode",
    action="store",
    type=str,
