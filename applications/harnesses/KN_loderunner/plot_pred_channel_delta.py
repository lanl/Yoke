...
def get_args():
    ...
    parser.add_argument(
        "--ckpt",
        type=str,
        default=f"runs/study_{RUN_ID}/study{RUN_ID}_modelState_epoch0089.pth",
    )
...
def main():
    ...
    for step in range(args.n_future_steps):
        ...
        with torch.no_grad():
            ...
            pred_delta_img = model(x_noisy, in_vars, out_vars, future_Dt)
    ...
    x_pred = context_seq.to(device)
    x_true = context_seq.to(device)

    # Initial context window for plotting
    context_means = context_seq.mean(dim=(2, 3))[0].detach().cpu().numpy()
    prefix = [context.mean().item() for context in context_means]
    ...
    for step in range(args.n_future_steps):
        ...
        # --------------------------------------------------------
        # Hardcoded nonsense test for autoregressive dependence
        # --------------------------------------------------------
        DEBUG_AUTOREG_NONSENSE = False
        DEBUG_START_STEP = 1
        DEBUG_VALUE = 10.0

        x_model_input = x_pred

        if DEBUG_AUTOREG_NONSENSE and step >= DEBUG_START_STEP:
            x_model_input = x_pred.clone()
            x_model_input[:, -1:] = DEBUG_VALUE

        with torch.no_grad():
            pred_delta_img = model(x_model_input, in_vars, out_vars, future_Dt)
    ...
    for step in range(args.n_future_steps):
        ...
        # Update autoregressive prediction context
        x_pred = torch.cat(
            [x_pred[:, 1:], pred_next_img.detach()],
            dim=1,
        )

        # Update true context separately
        x_true = torch.cat(
            [x_true[:, 1:], true_next_img.detach()],
            dim=1,
        )
    ...
    plt.savefig(
        f"study{RUN_ID}_pred_vs_truth_channel_delta_autoreg_clean.png",
        dpi=200,
    )
