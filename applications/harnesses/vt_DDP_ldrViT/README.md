LodeRunner-ViT Training - DDP - Venadito
========================================
...
Venadito cluster. A single timestep of the per-material density and velocity
fields is input and a single (offset) timestep is predicted.
...
- `ddp_ldrViT_study.csv` — learning-rate sweep for the 20-channel ViT model.

Study
-----

`ddp_ldrViT_study.csv` fixes the `LodeRunnerViT` architecture
(`VIT_EMBED_DIM=512`, `VIT_NUM_LAYERS=12`, `VIT_NUM_HEADS=8`) and sweeps the anchor
learning rate (`ANCHOR_LR`) across five decades from `1e-6` to `1e-2` to locate a
stable training regime. Model scalings are drawn from the `bomberman.py` smoketest.
...
```bash
yoke-start-study --csv ddp_ldrViT_study.csv --submissionType slurm
```
