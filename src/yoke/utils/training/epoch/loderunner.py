        ...
        for trainbatch_ID, traindata in enumerate(training_data):
            ...
            if seq:  #all_losses??
                gt_seq, pred_seq, train_losses = train_DDP_loderunner_seq_datastep(
                    ...
                    scheduled_prob=1.0,   # start with pure teacher forcing
                )
            else:
                # Perform a single training step
                truth, pred, train_losses = train_DDP_loderunner_seq_channel_datastep(
            ...
            with torch.no_grad():
                for valbatch_ID, valdata in enumerate(validation_data):
                    ...
                    if seq:  #all_losses??
                        ...
                        # Perform a single training step
                        end_img, pred_img, val_losses = eval_DDP_loderunner_seq_channel_datastep(
...
def train_DDP_loderunner_epoch_seq_context(
    training_data: torch.utils.data.DataLoader,
    validation_data: torch.utils.data.DataLoader,
    num_train_batches: int,
    num_val_batches: int,
    ...
    model.train()
    train_rcrd_filename = train_rcrd_filename.replace("<epochIDX>", f"{epochIDX:04d}")
    max_train_batches = min(num_train_batches, len(training_data))
    ...
    with (
        ...
        for trainbatch_ID, traindata in enumerate(training_data):
            # Stop when number of training batches is reached
            if trainbatch_ID >= max_train_batches:
                break

            if seq:  #all_losses??
                gt_seq, pred_seq, train_losses = train_DDP_temporal_loderunner_datastep(
                    data=traindata,
                    model=model,
                    optimizer=optimizer,
                    loss_fn=loss_fn,
    ...
    if epochIDX % train_per_val == 0:
        ...
        val_rcrd_filename = val_rcrd_filename.replace("<epochIDX>", f"{epochIDX:04d}")
        model.eval()
        max_val_batches = min(num_val_batches, len(val_data))

        with (open(val_rcrd_filename, "a") if rank == 0 else nullcontext()) as val_rcrd_file:
            with torch.no_grad():
                for valbatch_ID, valdata in enumerate(validation_data):
                    if valbatch_ID >= max_val_batches:
                        break

                    if seq:
                        end_img, pred_img, val_losses = eval_DDP_loderunner_seq_context_datastep(
                            data=valdata,
                            model=model,
                            loss_fn=loss_fn,
                            device=device,
    ...
    if epochIDX % train_per_val == 0:
        ...
        with (
            ...
            with torch.no_grad():
                ...
                if seq:  #all_losses??
                    gt_seq, pred_seq, train_losses = eval_DDP_loderunner_seq_datastep(
                        data=valdata,
                        model=model,
                        loss_fn=loss_fn,
                        device=device,
                    ...
                    # Perform a single training step
                    emd_img, pred_img, val_losses = eval_DDP_loderunner_datastep(
                        valdata, model, loss_fn, device, rank, world_size
                    )
