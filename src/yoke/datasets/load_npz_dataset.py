...
def read_npz_nan(npz: str | np.lib.npyio.NpzFile, field: str) -> np.ndarray:
...
class LabeledData:
    ...
    def cylex_data_loader(self) -> None:
        ...
        self.active_npz_field_names = np.append(
            self.active_npz_field_names, ["density_wall", "density_" + non_he_mats[1]]
        )
        ...
        if self.thermodynamic_variables in ("density and pressure", "all"):
            ...
            self.active_npz_field_names = np.append(
                self.active_npz_field_names, ["energy_wall", "energy_" + non_he_mats[1]]
            )
            self.active_hydro_field_names = np.append(
                self.active_hydro_field_names,
    ...
    if len(unique_channels) < len(channel_map):
        ...
        for i in np.arange(img_list_combined.shape[0]):
            result = combine_by_number_and_label(
                channel_map, img_list_combined[i], active_hydro_field_names
            )
        ...
        while prefix_attempt < 5:
            ...
            while attempt < self.max_file_checks:
                ...
                try:
                    ...
                    fields_start = [
                        f for f, keep in zip(active_npz_field_names, m_start) if keep
                    ]
                    chmap_start = [cm for cm, keep in zip(channel_map, m_start) if keep]
                    names_start = [
                        nm for nm, keep in zip(active_hydro_field_names, m_start) if keep
                    ]
                    if not fields_start:
                        start_npz.close()
                        end_npz.close()
                    ...
                    filtered_chmap = [
                        cm for cm, keep in zip(chmap_start, m_both) if keep
                    ]
                    filtered_names = [
                        nm for nm, keep in zip(names_start, m_both) if keep
                    ]
                    ...
                    start_img = (
                        torch.tensor(
                            np.stack(img_list_combined[0], axis=0), dtype=torch.float32
                        )
                        .contiguous()
                        .clone()
                    )
                    end_img = (
                        torch.tensor(
                            np.stack(img_list_combined[1], axis=0), dtype=torch.float32
                        )
                        .contiguous()
                        .clone()
                    )
                    ...
                    if self._dbg_cnt < 10 and _current_rank() == 0:
                        ...
                        def _legend_line(idxs):
                            names = []
                            L = (
                                len(self.all_hydro_field_names)
                                if hasattr(self, "all_hydro_field_names")
                                else 0
                            )
                            for i in idxs:
                                if 0 <= i < L:
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for file_path in file_paths:
            try:
                ...
                field_tensor = (
                    torch.tensor(np.stack(field_imgs, axis=0), dtype=torch.float32)
                    .contiguous()
                    .clone()
                )
